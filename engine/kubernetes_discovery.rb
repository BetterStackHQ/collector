...
class KubernetesDiscovery
  ...
  def initialize(working_dir)
    ...
    @base_dir = "#{working_dir}/kubernetes-discovery"
    @last_run_time = nil
    @node_name = ENV['HOSTNAME']
  end
  
  def should_discover?
    latest_vector_yaml = "#{@working_dir}/latest-valid-vector.yaml"
  ...
  def run
    ...
    begin
      changed = discover_and_update
      puts "Kubernetes discovery completed (changed: #{changed})"
      changed
    rescue => e
  ...
  def cleanup_old_versions(keep_count = 5)
    versions = Dir.glob("#{@base_dir}/*").select { |f| File.directory?(f) }
    ...
    namespaces.each do |namespace|
      ...
      services.each do |service|
        ...
        endpoints.each do |endpoint|
          # Create unique key for deduplication: pod_ip:port:path
          endpoint_key = "#{endpoint[:pod_ip]}:#{endpoint[:endpoint]}"
          next if discovered_endpoints.include?(endpoint_key)
          
          discovered_endpoints.add(endpoint_key)
          config = generate_config(endpoint)
          if config
            filepath = "#{new_dir}/#{config[:filename]}"
