...
- **eBPF** (`ebpf/Dockerfile`) — OBI + Node Agent + Better Stack eBPF Agent + Cluster Agent + Node Exporter + Postgres Exporter on Debian, provisioned by the collector's bootstrap
...
ebpf/
  Dockerfile                # Multi-stage: OBI 0.10.0 + Node Agent 1.30.0 + eBPF Agent 0.1.6 + exporters + Debian 13.5-slim
  bootstrap_supervisord.conf
  run_supervisord.sh
swarm/
...
- **eBPF → Collector**: Via host network mode
  ...
  - Runtime-delivered supervisor configuration selects either Node Agent or Better Stack eBPF Agent
